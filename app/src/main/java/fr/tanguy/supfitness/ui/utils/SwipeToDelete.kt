...
import fr.tanguy.supfitness.ui.weight.WeightObject.removeItem
...
import fr.tanguy.supfitness.ui.weight.WeightObject
...
class SwipeToDelete(mContext: Context, private val mAdapter: WeightAdapter) :
    ...
    override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
        println("SIZE BEFORE DELETE : ${WeightObject.getAllWeights().size}")

        val position = viewHolder.adapterPosition
        println("POSITION AFTER DELETE : $position")

        removeItem(position)
        mAdapter.notifyItemRemoved(position)

        println("SIZE AFTER DELETE : ${WeightObject.getAllWeights().size}")
    }
